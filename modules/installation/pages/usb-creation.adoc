...
. Extract the fawkes LiveCD and the Crucible package from the tar ball
+
[source,bash]
----
tar --wildcards -xzvf ./fawkes-${RELEASE}.tar.gz "*/images/fawkes-live/fawkes-live*.iso*" "*/rpm/*/crucible*.rpm"
----
. Validate the liveCD
+
[source,bash]
----
cd "fawkes-${RELEASE}/images/fawkes-live/"
sha256sum -c ./fawkes-live*sha256.txt
cd -
----
This should return with:
+
....
fawkes-live-<image ID>-x86_64.iso: OK
....
. Install Crucible
+
